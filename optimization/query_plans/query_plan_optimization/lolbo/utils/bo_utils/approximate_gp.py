...
class ApproximateGPyTorchModel(GPyTorchModel):
    ...
    def fantasize(self, X, sampler=MCSampler, observation_noise=True, *args, **kwargs):
        raise NotImplementedError("Fantasization of approximate GPs has not been implemented yet.")
        ...
        if type(inducing_points) is not Tensor:
            ...
            if init_method == "pivchol":
                with torch.no_grad():
                    ...
                    inducing_points = _pivoted_cholesky_init(train_X, train_train_kernel, max_length=inducing_points)
                ...
                raise (AssertionError("Only pivoted cholesky initialization has been implemented."))
