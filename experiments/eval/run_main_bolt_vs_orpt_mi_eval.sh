...
CONFIG=peptide_experiment/configs/peptide_main_eval_gpu0.yaml  # any shard's pointer config works; see README's "0. Prerequisite"
RESULTS=experiments/eval/results/main_bolt_vs_orpt_mi
...
echo "[run_main_bolt_vs_orpt_mi_eval] step 0: train MTBO/GP-expert-transfer/OptFormer baselines into runs/peptide_main_bolt (cheap, idempotent, single GPU, serial -- no training step for LLAMBO)"
python -m peptide_experiment.cli train_mtbo --config peptide_experiment/configs/peptide_main_bolt.yaml \
    > experiments/eval/logs/main_bolt_vs_orpt_mi_step0_mtbo.log 2>&1
python -m peptide_experiment.cli train_gp_expert_transfer --config peptide_experiment/configs/peptide_main_bolt.yaml \
    > experiments/eval/logs/main_bolt_vs_orpt_mi_step0_gp_expert_transfer.log 2>&1
python -m peptide_experiment.cli train_optformer --config peptide_experiment/configs/peptide_main_bolt.yaml \
    > experiments/eval/logs/main_bolt_vs_orpt_mi_step0_optformer.log 2>&1

echo "[run_main_bolt_vs_orpt_mi_eval] step 1: raw proposal generation (7-way GPU parallel, task_set=${TASK_SET}, BOLT/ORPT-MI/OptFormer only -- MTBO/POGPE/SGPE/LLAMBO have no LLM checkpoint to sample raw proposals from)"
pids=()
for gpu in 0 1 2 3 4 5 6; do
    nohup python experiments/eval/generate_raw_proposals.py \
        --config peptide_experiment/configs/peptide_main_eval_gpu${gpu}.yaml \
        --manifest experiments/eval/manifests/main_bolt_vs_orpt_mi__gpu${gpu}.yaml \
        --task-sets ${TASK_SET} \
        > experiments/eval/logs/main_bolt_vs_orpt_mi_step1_gpu${gpu}.log 2>&1 &
    pids+=("$!")
done
wait_all "${pids[@]}"

echo "[run_main_bolt_vs_orpt_mi_eval] step 2: incumbent vs pool size (no BO, 7-way parallel, BOLT/ORPT-MI/OptFormer only)"
pids=()
for gpu in 0 1 2 3 4 5 6; do
    nohup python experiments/eval/incumbent_vs_pool_size.py \
        ...
        --manifest experiments/eval/manifests/main_bolt_vs_orpt_mi__gpu${gpu}.yaml \
        --task-sets ${TASK_SET} \
        > experiments/eval/logs/main_bolt_vs_orpt_mi_step2_gpu${gpu}.log 2>&1 &
    pids+=("$!")
done
wait_all "${pids[@]}"
...
python experiments/eval/plot_incumbent_vs_pool_size.py \
    ...
    > experiments/eval/logs/main_bolt_vs_orpt_mi_step2_combined.log 2>&1

echo "[run_main_bolt_vs_orpt_mi_eval] step 3: fixed-target pool BO (real LOLBO -- expensive, 8-way parallel, all 6 arms)"
# Sanity check first, on one GPU shard, before the full sweep (uncomment):
#   python experiments/eval/fixed_target_rejection_bo.py \
#       --config peptide_experiment/configs/peptide_main_eval_gpu0.yaml \
#       --manifest experiments/eval/manifests/main_bolt_vs_orpt_mi__gpu0.yaml \
#       --task-sets ${TASK_SET} --target-pool-sizes 10 --limit-tasks 1
# Or for the new milestone-independent baselines specifically:
#   python experiments/eval/fixed_target_rejection_bo.py \
#       --config peptide_experiment/configs/peptide_main_eval_gpu7.yaml \
#       --manifest experiments/eval/manifests/main_bolt_vs_orpt_mi__gpu7.yaml \
#       --task-sets ${TASK_SET} --target-pool-sizes 10 --limit-tasks 1
pids=()
for gpu in 0 1 2 3 4 5 6 7; do
    nohup python experiments/eval/fixed_target_rejection_bo.py \
        ...
        > experiments/eval/logs/main_bolt_vs_orpt_mi_step3_gpu${gpu}.log 2>&1 &
    pids+=("$!")
done
wait_all "${pids[@]}"

# --arms excludes POGPE/SGPE: their milestone field means n_experts (5/10/20),
# not a real training milestone -- plotting them against the 126-900 axis
# would render as misleading clutter, not a real line (see gpu7 manifest's
# comment). BOLT/ORPT-MI/OptFormer/MTBO share the real milestone axis;
# LLAMBO is milestone-independent but safe to include (plot_arm_line already
# renders single-milestone arms as a flat dashed reference line).
python experiments/eval/plot_fixed_target_rejection_bo.py \
    --results-dir "$RESULTS_GPUS_ALL" --out-dir "$RESULTS" --bo-calls ${BO_CALLS} \
    --arms BOLT,ORPT-MI,OptFormer,MTBO,LLAMBO \
    > experiments/eval/logs/main_bolt_vs_orpt_mi_step3_combined.log 2>&1
...
echo "[run_main_bolt_vs_orpt_mi_eval] done -- see ${RESULTS}/plots/"
