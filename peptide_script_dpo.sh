...
do
    ...
    if [ "$var" -eq 1 ]; then
        ...
        SAMPLE_MODEL_PATH="$PEPTIDE_CKPT_DIR/epoch_4"
        CUDA_VISIBLE_DEVICES=1 python3 sampling_transformers.py --model-path "$SAMPLE_MODEL_PATH" --start-index $((var+1)) --num-peptides 1 --output-file ./sampled_output_from_ft/sample_output_transformers_$var.jsonl
    ...
    PEPTIDE_CKPT_DIR="/home/mok/module/steering/BOLT/fine-tuning/peptides/output/qwen_2_5_3B_dpo_output_${var}"
    
    ##### DPO
    CUDA_VISIBLE_DEVICES=1 tune run --nnodes 1 --nproc_per_node 1 full_dpo_distributed --config torchtune_config/qwen_2_5_3B_dpo.yaml output_dir="$PEPTIDE_CKPT_DIR" dataset.data_files="$DPO_JSONL"
done
